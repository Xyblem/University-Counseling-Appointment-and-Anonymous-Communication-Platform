    ...
    return (<div style={{marginLeft: "25px"}}>
        ...
        <ResponseHandler<CloseAccountRequest, any>
            ...
            networkErrorComponent={
                <div>
                    <h3>网络错误</h3>
                    <p className="home-error-detail">{closeAccountState?.networkError?.message}</p>
                </div>
            }

            finishedComponent={
                <div>
                    <h3>注销账号{ReturnObject.Status.ChineseName.get(closeAccountState?.returnObject?.status)}</h3>
                    {closeAccountState?.returnObject?.status===ReturnObject.Status.SUCCESS ? (
                        <Button type="primary" block onClick={() => {
                             "/auth/login";
                        }}>返回登录界面</Button>
                    ): (<p className="home-error-detail">{closeAccountState?.returnObject?.message}</p>)}
                </div>
            }
        />
    </div>)
