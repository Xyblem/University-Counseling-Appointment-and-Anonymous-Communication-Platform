...
export const MyPsychKnowledgeTeacherForm:React.FC = () => {
    ...
    const MyPsychKnowledgeHandlerRef=useRef<ResponseHandlerRef<{teacherUsername:string}, PsychKnowledgeDTO[]>>(null);
    ...
    return (<div className="layout-flex-column">
        ...
        <ResponseHandler<{teacherUsername:string}, PsychKnowledgeDTO[]>
            ...
            autoRequest={{teacherUsername:context.user==null?"":context.user.username}}
            ...
            finishedComponent={(!(myPsychKnowledgeState?.returnObject?.status === ReturnObject.Status.SUCCESS)) ? (
                <div>
                    ...
                    <div className="post-container">{PsychKnowledgeCardList}</div>:<p>你还没有发布任何科普</p>
            )}
        />
    </div>)
}